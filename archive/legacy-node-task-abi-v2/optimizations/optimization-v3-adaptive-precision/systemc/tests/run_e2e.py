...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="systemc-e2e-") as directory:
        ...
        assert summary["config"]["precision_policy"] == "uniform_fraction"
        assert summary["config"]["frac_bits"] == 26
        assert summary["stability"]["fraction_retry_nodes"] == 0
        assert "precision_assisted_nodes" in summary["stability"]
        for filename in (
            "nodes.csv",
            "operations.csv",
            "memory.csv",
        ...
        node_columns = (result / "nodes.csv").read_text().splitlines()[0]
        for column in (
            "l_frac_bits",
            ...
            assert column in node_columns.split(",")
        manifest = json.loads((artifact / "manifest.json").read_text())
        first_meta = manifest["nodes"]["0"]["node_meta"]
        final_image = (result / "final_memory_image.bin").read_bytes()
        meta = final_image[
            first_meta["offset"] :
            first_meta["offset"] + first_meta["size"]
        ]
        assert meta[0] == 1
        assert meta[1] & ~0x07 == 0
        assert meta[7] == 26

        retry_config = root / "retry.json"
        retry_values = dict(
            base_config := json.loads(config.read_text()),
            frac_bits=20,
            adaptive_frac_retry=True,
            retry_frac_bits=26,
            fixed_factor_rel_tol=0.0,
        )
        retry_config.write_text(json.dumps(retry_values))
        retry_summary = run_simulator(
            simulator,
            artifact,
            retry_config,
            root / "retry",
        ...
        assert retry_summary["status"] == "ok"
        assert (
            retry_summary["config"]["precision_policy"]
            == "adaptive_fraction_retry"
        )
        assert retry_summary["stability"]["fraction_retry_nodes"] > 0
