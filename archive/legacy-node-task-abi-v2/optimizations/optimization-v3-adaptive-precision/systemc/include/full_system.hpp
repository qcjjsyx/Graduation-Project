            ...
            try {
                ...
                if (mode_has_fixed(mode_)) {
                    const auto run_fixed =
                        [&](const ModelConfig& factor_config) {
                            return factor_config.bfp_tile_size == 0 ?
                                factor_fixed_front(
                                    node.assembled_q,
                                    work.task.total_dim,
                                    work.task.pivot_dim,
                    ...
                    const auto run_fp64_rescue =
                        [&](const ModelConfig& quantization_config) {
                            ...
                            const auto rescue = factor_fp64_front(
                                ...
                                work.task.pivot_dim,
                                rescue_config);
                            auto quantized = quantize_rescued_factor(
                                rescue,
                                node.task.total_dim,
                                node.task.pivot_dim,
                                quantization_config);
                    ...
                    try {
                        fixed_result = run_fixed(config_);
                    } catch (const FactorCheckFailure&) {
                        if (config_.adaptive_frac_retry &&
                            config_.bfp_tile_size != 0 &&
                            config_.retry_frac_bits >
                                config_.frac_bits) {
                            auto retry_config = config_;
                            retry_config.frac_bits =
                                config_.retry_frac_bits;
                            try {
                                fixed_result =
                                    run_fixed(retry_config);
                                fixed_result->factor
                                    .precision_assisted = true;
                                fixed_result->factor
                                    .fraction_retry_attempted = true;
                            } catch (
                                const PrecisionRescueRequired&) {
                                if (config_.fixed_rescue_mode !=
                                    "fp64") {
                                    throw;
                                }
                                fixed_result =
                                    run_fp64_rescue(retry_config);
                                fixed_result->factor
                                    .fraction_retry_attempted = true;
                            }
                        } else {
                            if (config_.fixed_rescue_mode != "fp64") {
                                throw;
                            }
                            fixed_result =
                                run_fp64_rescue(config_);
                        }
                ...
                if (fixed_result) {
                    ...
                    if (node.fixed.fraction_retry_attempted) {
                        ++node.quant_stats.fraction_retry_count;
                    }
                    if (node.fixed.precision_assisted) {
                        ++node.quant_stats.precision_assist_count;
                    }
                }
                ...
                if (fixed_result &&
                    node.fixed.fraction_retry_attempted) {
                    auto retry_operations = build_operation_plan(
                        work.task.node_id,
                        work.task.total_dim,
                        work.task.pivot_dim,
                        config_,
                        stats_.cycle);
                    const auto retry_end_cycle =
                        schedule_operations(
                            retry_operations,
                            config_,
                            stats_.cycle);
                    while (stats_.cycle < retry_end_cycle) {
                        tick_wait();
                    }
                    stats_.operations.insert(
                        stats_.operations.end(),
                        retry_operations.begin(),
                        retry_operations.end());
                    stats_.timeline(
                        event_text(
                            "fixed_frac_retry",
                            work.task.node_id,
                ...
                perf.l_frac_bits = node.fixed.l_frac_bits;
                perf.fraction_retry_attempted =
                    node.fixed.fraction_retry_attempted;
                perf.precision_assisted =
                    node.fixed.precision_assisted;
                perf.precision_rescued =
...
private:
    ...
    std::uint64_t write_fixed(const ManifestNode& meta, NodeStorage& node) {
        ...
        if (node_meta.size() >= 4) {
            node_meta[0] = 1;
            node_meta[1] = static_cast<std::uint8_t>(
                (node.fixed.precision_rescued ? 0x01u : 0u) |
                (node.fixed.precision_assisted ? 0x02u : 0u) |
                (node.fixed.fraction_retry_attempted ? 0x04u : 0u));
            node_meta[2] =
            ...
            if (node_meta.size() >= 16) {
                ...
                node_meta[6] = static_cast<std::uint8_t>(
                    node.fixed.tile_size);
                node_meta[7] = static_cast<std::uint8_t>(
                    node.fixed.l_frac_bits);
                put_u32(
                    8,
