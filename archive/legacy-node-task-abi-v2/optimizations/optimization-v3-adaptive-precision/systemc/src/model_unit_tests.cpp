...
void test_tile_bfp_qau_and_factor() {
    ...
    expect(
        factor.factor.l_frac_bits == config.frac_bits,
        "tile-BFP factor did not retain its L QF precision");

    hw::ModelConfig inexact{};
    inexact.bfp_tile_size = 16;
    inexact.q_use_bits = 7;
    inexact.frac_bits = 2;
    inexact.workspace_guard_bits = 0;
    inexact.fixed_pivot_rel_tol = 0.0;
    inexact.fixed_factor_rel_tol = 0.0;
    bool factor_check_failed = false;
    try {
        (void)hw::factor_fixed_front_tile_bfp(
            {
                3, 1,
                1, 1,
            },
            ...
            2,
            inexact);
    } catch (const hw::FactorCheckFailure&) {
        factor_check_failed = true;
    }
    expect(
        factor_check_failed,
        "tile-BFP local error gate must expose a retryable failure");
}
