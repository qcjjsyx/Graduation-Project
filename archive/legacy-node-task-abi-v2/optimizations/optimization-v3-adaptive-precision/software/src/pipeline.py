...
class PipelineOutputs:
    ...
    original_solution_reference_path: Path
    row_scale_exponents_path: Path
    column_scale_exponents_path: Path
...
def run_pipeline(config: PipelineConfig) -> PipelineOutputs:
    ...
    column_scales = np.exp2(
        equilibration.column_scale_exponents.astype(np.float64)
    )
    solution_scaled = solution_original / column_scales
    ...
    solution_permuted = solution_scaled[permutation]
    ...
    original_solution_reference_path = out_dir / "original_solution_f64.bin"
    row_scale_exponents_path = out_dir / "row_scale_e.bin"
    column_scale_exponents_path = out_dir / "column_scale_e.bin"
    ...
    source_exponent_counts: List[int] = []
    ...
    for node_range, front_indices in zip(symbolic.node_ranges, symbolic.front_indices):
        ...
        source_exponent_counts.append(int(source.tile_exponents.size))
        quant_stats.append(source.stats)
        clip_total += source.stats.clip_count
        sat_total += source.stats.sat_count

    # The solve ABI intentionally keeps one RHS exponent.  Tile BFP applies to
    # frontal matrices and factors; it does not alter the global vector ABI.
    rhs_quant = replace(config.quant, bfp_tile_size=0)
    rhs_source = quantize_local_contribution(
        rhs_permuted.reshape(-1, 1), rhs_quant
    )
    ...
    original_solution_reference_path.write_bytes(
        solution_original.astype("<f8", copy=False).tobytes(order="C")
    )
    ...
    column_scale_exponents_path.write_bytes(
        equilibration.column_scale_exponents.astype(
            "<i2", copy=False
        ).tobytes(order="C")
    )
    ...
    ) = estimate_work_region_sizes(
        symbolic.node_ranges,
        symbolic.front_indices,
        config.quant.bfp_tile_size,
    )
    ...
    manifest = build_manifest(
        ...
        source_exponent_counts=source_exponent_counts,
        ...
        output_sizes={
            ...
            "original_solution_f64.bin":
                original_solution_reference_path.stat().st_size,
            "row_scale_e.bin": row_scale_exponents_path.stat().st_size,
            "column_scale_e.bin":
                column_scale_exponents_path.stat().st_size,
    ...
    return PipelineOutputs(
        ...
        original_solution_reference_path=original_solution_reference_path,
        row_scale_exponents_path=row_scale_exponents_path,
        column_scale_exponents_path=column_scale_exponents_path,
...
def estimate_work_region_sizes(
    ...
    for node_range, front in zip(node_ranges, front_indices):
        ...
        update_e_sizes.append(
            _tile_exponent_count(update_dim, update_dim, bfp_tile_size) * 2
            if update_dim
            else 0
        )
        ...
        u_tile_count = _tile_exponent_count(
            pivot_dim, total_dim, bfp_tile_size
        )
        # Header: validity/rescue, scalar compatibility exponents, tile size,
        # U exponent count and update exponent count. U tile exponents follow.
        node_meta_sizes.append(max(64, 16 + u_tile_count * 2))
    ...
    *,
    ...
    return {
        ...
        "verification": {
            ...
            "original_solution_reference_file":
                "original_solution_f64.bin",
        ...
        "equilibration": {
            ...
            "equation": (
                ...
                if config.equilibration.mode
                in {"pow2-row-column", "pow2-ruiz"}
                else "D_r * A * x = D_r * b"
            ),
            "solution_requires_unscale":
                config.equilibration.mode
                in {"pow2-row-column", "pow2-ruiz"},
            "row_scale_exponent_file": "row_scale_e.bin",
            "row_scale_exponent_dtype": "int16",
            "row_scale_exponent_count": actual_matrix_dim,
            "column_scale_exponent_file": "column_scale_e.bin",
            "column_scale_exponent_dtype": "int16",
            "column_scale_exponent_count": actual_matrix_dim,
            "row_max_before": {
            ...
            "column_max_before": {
                "min_nonzero": equilibration.column_max_before_min,
                "max": equilibration.column_max_before_max,
            },
            "column_max_after": {
                "min_nonzero": equilibration.column_max_after_min,
                "max": equilibration.column_max_after_max,
            },
        },
        "symbolic": {
            "node_count": len(symbolic.node_ranges),
        ...
        "quantization": {
            "format": (
                "tile_bfp_int32"
                if config.quant.bfp_tile_size
                else "S_format_local_contribution"
            ),
            "software_role": (
            ...
            "bfp_tile_size": config.quant.bfp_tile_size,
        ...
        "nodes": {
            str(node_id): {
                ...
                "local_source": {
                    ...
                    "exponent_count": source_exponent_counts[node_id],
                    "tile_size": config.quant.bfp_tile_size,
...
def _tile_exponent_count(rows: int, cols: int, tile_size: int) -> int:
    if rows == 0 or cols == 0:
        return 0
    if tile_size == 0:
        return 1
    return _ceil_div(rows, tile_size) * _ceil_div(cols, tile_size)
