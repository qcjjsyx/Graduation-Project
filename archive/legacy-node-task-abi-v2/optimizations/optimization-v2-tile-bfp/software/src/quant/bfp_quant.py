...
def quantize_local_contribution(
    ...
    if config.bfp_tile_size:
        tile_size = config.bfp_tile_size
        tile_rows = _ceil_div(matrix.shape[0], tile_size)
        tile_cols = _ceil_div(matrix.shape[1], tile_size)
        mantissa = np.zeros(matrix.shape, dtype=np.int32)
        tile_exponents = np.zeros((tile_rows, tile_cols), dtype=np.int16)
        clip_count = 0
        sat_count = 0
        for tile_row in range(tile_rows):
            row_start = tile_row * tile_size
            row_end = min(row_start + tile_size, matrix.shape[0])
            for tile_col in range(tile_cols):
                col_start = tile_col * tile_size
                col_end = min(col_start + tile_size, matrix.shape[1])
                tile = matrix[row_start:row_end, col_start:col_end]
                tile_bound = _clip_bound(tile, config.clip_percentile)
                if tile_bound == 0.0:
                    tile_exponent = 0
                    tile_q = np.zeros(tile.shape, dtype=np.int32)
                else:
                    tile_exponent = _scale_exponent(tile_bound, q_limit)
                    clipped = np.clip(tile, -tile_bound, tile_bound)
                    scaled = np.rint(clipped / np.ldexp(1.0, tile_exponent))
                    tile_q = np.clip(scaled, -q_limit, q_limit).astype(np.int32)
                    clip_count += int(np.count_nonzero(np.abs(tile) > tile_bound))
                    sat_count += int(np.count_nonzero(np.abs(tile_q) == q_limit))
                mantissa[row_start:row_end, col_start:col_end] = tile_q
                tile_exponents[tile_row, tile_col] = tile_exponent

        exponent = int(tile_exponents.reshape(-1)[0]) if tile_exponents.size else 0
        return QuantizedSource(
            mantissa=mantissa,
            exponent=exponent,
            tile_exponents=tile_exponents,
            tile_size=tile_size,
    ...
    return QuantizedSource(
        mantissa=mantissa,
        exponent=exponent,
        tile_exponents=np.asarray([[exponent]], dtype=np.int16),
        tile_size=0,
        shape=(int(matrix.shape[0]), int(matrix.shape[1])),
        stats=QuantizationStats(
            total_elements=int(matrix.size),
...
def dequantize_source(source: QuantizedSource) -> np.ndarray:
    ...
    restored = np.empty(source.shape, dtype=np.float64)
    for tile_row in range(source.tile_exponents.shape[0]):
        row_start = tile_row * source.tile_size
        row_end = min(row_start + source.tile_size, source.shape[0])
        for tile_col in range(source.tile_exponents.shape[1]):
            col_start = tile_col * source.tile_size
            col_end = min(col_start + source.tile_size, source.shape[1])
            restored[row_start:row_end, col_start:col_end] = (
                source.mantissa[row_start:row_end, col_start:col_end].astype(
                    np.float64
                )
                * np.ldexp(1.0, int(source.tile_exponents[tile_row, tile_col]))
            )
    return restored
...
def flatten_quantized_source(source: QuantizedSource) -> tuple[list[int], list[int]]:
    ...
    return (
        q_values,
        source.tile_exponents.reshape(-1).astype(np.int16).tolist(),
    )
