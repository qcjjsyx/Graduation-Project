...
private:
        ...
        case S_LOAD:
            if (in_valid.read() && in_ready.read()) {
                candidates_.push_back({
                    static_cast<std::uint16_t>(in_row_logical.read().to_uint()),
    ...
    void select_pivot() {
        if (candidates_.empty()) {
            // A failed search is still a valid response and uses the same
            // ready/valid handshake as a successful pivot result.
