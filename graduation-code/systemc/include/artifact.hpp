...
inline ArtifactManifest load_manifest(const std::filesystem::path& path) {
    ...
    if (equilibration.at("solution_requires_unscale").get<bool>()) {
        throw std::runtime_error(
            "SystemC supports row-only equilibration without solution unscale");
    }
...
inline void validate_regions(const ArtifactManifest& manifest) {
    ...
    for (std::uint32_t node_id = 0;
        ...
        if (node.front_q.size !=
            ...
            node.front_e.size != sizeof(std::int16_t) ||
            node.update_q.size !=
                static_cast<std::uint64_t>(update) * update *
                    sizeof(std::int32_t) ||
            node.update_e.size != (update == 0 ? 0 : sizeof(std::int16_t)) ||
            node.l_factor.size !=
