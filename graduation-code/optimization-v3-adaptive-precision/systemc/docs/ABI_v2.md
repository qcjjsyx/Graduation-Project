...
> 本文保留 128 字节 NodeTask 和基础 v2 说明。v3
...
- `L`：int32 QF-format，实际值为 `q × 2^-F`；v3 推荐默认 `F=26`
...
v3 默认 30。二者不需要相同，具体值分别记录在 manifest 和仿真配置中。

### node metadata

下表是标量 payload 的历史布局。v3 Tile BFP 的 flags、tile size、实际 L frac bits
和可变长 U exponent 表以
[Tile BFP ABI](../../docs/TILE_BFP_ABI.md) 为准：
