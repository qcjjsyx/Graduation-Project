...
inline nlohmann::json quant_stats_json(const QuantStats& value) {
    return {
        ...
        {"fraction_retry_count", value.fraction_retry_count},
        {"precision_assist_count", value.precision_assist_count},
    ...
    return {
        ...
        {"precision_policy",
         config.adaptive_frac_retry ?
             "adaptive_fraction_retry" : "uniform_fraction"},
        {"frac_bits", config.frac_bits},
        {"adaptive_frac_retry", config.adaptive_frac_retry},
        {"retry_frac_bits", config.retry_frac_bits},
...
inline void write_reports(
    ...
    std::uint64_t precision_assisted_nodes = 0;
    std::uint64_t fraction_retry_nodes = 0;
    ...
    for (const auto& [node_id, node] : stats.nodes) {
        ...
        precision_assisted_nodes +=
            node.quant.precision_assist_count;
        fraction_retry_nodes +=
            node.quant.fraction_retry_count;
    ...
    nlohmann::json summary{
        ...
        {"stability", {
            {"fraction_retry_nodes", fraction_retry_nodes},
            {"precision_assisted_nodes", precision_assisted_nodes},
...

    {
        ...
        output
            << "node_id,total_dim,pivot_dim,status,failure_reason,"
               ...
               "fixed_update_exponent,l_frac_bits,"
               "fraction_retry_attempted,precision_assisted,"
               "precision_rescued,assembled_tile_count,"
               ...
               "matrix_overflow_count,fraction_retry_count,"
               "precision_assist_count,precision_rescue_count,"
               "rescue_quantization_saturation_count,"
        ...
        for (std::uint16_t node_id = 0; node_id < manifest.node_count; ++node_id) {
            ...
            output << node_id << ',' << node.total_dim << ',' << node.pivot_dim
                   ...
                   << node.l_frac_bits << ','
                   << (node.fraction_retry_attempted ? 1 : 0) << ','
                   << (node.precision_assisted ? 1 : 0) << ','
                   << (node.precision_rescued ? 1 : 0) << ','
