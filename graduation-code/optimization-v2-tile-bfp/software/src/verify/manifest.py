...
def validate_manifest(manifest_path: str | Path) -> ManifestValidationResult:
    ...
    _validate_quantization(manifest)
...
def _validate_quantization(manifest: dict) -> None:
    nodes = manifest["nodes"]
    tile_size = _as_int(
        manifest.get("quantization", {}).get("bfp_tile_size", 0),
        "quantization.bfp_tile_size",
    )
    if tile_size not in {0, 16}:
        raise ManifestValidationError("bfp_tile_size must be 0 or 16")
    for node_id, node in nodes.items():
        ...
        expected_format = "tile_bfp_int32" if tile_size else "S_format"
        if local_source.get("format") != expected_format:
        ...
        front_e_count = _tile_exponent_count(front_dim, front_dim, tile_size)
        if (
            _as_int(node["front_e"]["size"], f"node_{node_id}_front_e_size")
            != front_e_count * 2
        ):
            raise ManifestValidationError(
                f"node {node_id} front_e tile exponent size mismatch"
            )
        if _as_int(
            local_source.get("exponent_count", -1),
            f"node_{node_id}_local_exponent_count",
        ) != front_e_count:
            raise ManifestValidationError(
                f"node {node_id} local exponent count mismatch"
            )
        if _as_int(
            local_source.get("tile_size", -1),
            f"node_{node_id}_local_tile_size",
        ) != tile_size:
            raise ManifestValidationError(
                f"node {node_id} local tile size mismatch"
            )
        ...
        update_e_count = _tile_exponent_count(
            update_dim, update_dim, tile_size
        )
        u_e_count = _tile_exponent_count(pivot_dim, front_dim, tile_size)
        expected_sizes = {
            "update_q": update_dim * update_dim * 4,
            "update_e": update_e_count * 2,
            "l_factor": front_dim * pivot_dim * 4,
            "u_factor": pivot_dim * front_dim * 4,
            "p_vector": pivot_dim * 2,
            "node_meta": max(64, 16 + u_e_count * 2),
...
def _tile_exponent_count(rows: int, cols: int, tile_size: int) -> int:
    ...
    if tile_size == 0:
        return 1
    return ((rows + tile_size - 1) // tile_size) * (
